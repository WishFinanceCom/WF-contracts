...
contract Lockup12mEmitter is MultiEventsHistoryAdapter {
...
contract Lockup12m is Object {
    ...
    function Lockup12m(address _asset) {
        asset = _asset;
    }
    ...
    function _error(uint _errorCode, bytes32 _message) internal returns(uint) {
        Lockup12mEmitter(eventsHistory).emitError(_message);
        return _errorCode;
    }
    ...
    function payIn() onlyContractOwner returns(uint errorCode) {
        ...
        lock = accountData(amount,now + 1 years);
        return OK;
    }
